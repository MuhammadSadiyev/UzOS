...
inline bytes::vector Sha256(bytes::const_span a, bytes::const_span b) {
	QCryptographicHash hash(QCryptographicHash::Sha256);
	hash.addData(reinterpret_cast<const char*>(a.data()), a.size());
	hash.addData(reinterpret_cast<const char*>(b.data()), b.size());
	const auto qres = hash.result();
	auto res = bytes::vector(qres.size());
	std::memcpy(res.data(), qres.constData(), qres.size());
	return res;
}
} // namespace openssl
